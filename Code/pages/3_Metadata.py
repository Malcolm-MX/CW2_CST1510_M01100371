...
#Extra feature for this page is a sidebar filter that lets the user pick out a specific dataset page an through that they can see the visual data about that dataset
with st.sidebar:
    st.header("Navigation")
    name_ = st.selectbox("Name Of Dataset",data['name'].unique())

#Converted upload_date from plain text as it's stored in SQLite into a real
#datetime type, so it can be sorted or handled correctly if needed and also helps visually
data['upload_date'] = pd.to_datetime(data['upload_date'])

#Narrow the data down to just the dataset selected in the sidebar
...
with col3:
    ...
    #set_index('name') makes each bar label with the dataset name instead
    #of a meaningless row number
